...
Any bug or vulnerability to report? An idea of feature you'd like to see in Amusix? https://github.com/Tilianh/Amusix/issues/new[Create a new bug report or a feature request in GitHub.]
...
. Merge all branches corresponding to features and fixes to include in the version into the created `release` branch
. Increment the application version in the `package.json` file at the root of the frontend application `AmusixFrontapp` (<<version_numbering,check how to number a version>>)
. When everything is ready, merge the `release` branch into `main` via a pull request

When the merge is triggered from the created pull request, the following steps will be automatically executed:

. Frontend documentation generation and deployment in GitHub Pages
. Docker images building and pushing in Docker Hub
. Application deployment on the production server via the Docker container architecture
. New GitHub release drafting

==== Version numbering [[version_numbering]]
